...
try:
    ...
    print("Install with: pip install flask flask-cors flask-limiter flask-caching")
    exit(1)

# Try importing agentql which is used for some flows
try:
    import agentql
    ...
    print("Install with: pip install agentql playwright")
    # Set placeholder variables so the code still runs
    agentql = None
    async_playwright = None

# For optional playwright install
try:
    from playwright.async_api import async_playwright
except ImportError:
    async_playwright = None
    print("Warning: playwright not installed. RPA flows will not work.")
...
conversation_context = {
    ...
    "user_data": {
        ...
        "data_usage": 5.0,  # Default data usage in GB
        "current_provider": "",
        "open_to_switching": True
    },
    "last_message": "",
...
def search():
    ...
    # If state is greeting: This is the initial greeting, respond accordingly
    if conversation_context["state"] == "greeting":
        conversation_context["state"] = "awaiting_yes_no"
        response = "Hello! Welcome to our mobile phone plan assistance service. I'm Blue, your AI assistant here to help you find the perfect mobile plan. Would you like to provide some details about what you're looking for?"
        return jsonify({"response": response})

    # If awaiting yes/no to the greeting
    elif conversation_context["state"] == "awaiting_yes_no":
        if "yes" in user_message or user_message in ["y", "yeah", "sure", "ok"]:
            conversation_context["state"] = "collecting_details"
            response = "Great! Please provide your plan details in the form below."
            return jsonify({
                "response": response,
                "show_form": True  # Signal to the frontend to show the form
            })
        elif "no" in user_message or user_message in ["n", "nope"]:
            conversation_context["state"] = "casual_conversation"
            llama_response = llama_client.generate_response(
                prompt="User said they don't want to provide plan details right now. Respond in a friendly way and ask what they'd like to know about mobile plans instead.",
            ...
            # If user says something else, let Llama understand and respond
            # but still guide them to yes/no
            llama_response = llama_client.generate_response(
                prompt=f"User: {user_message}\nBlue:",
                system=f"{BLUE_SYSTEM_PROMPT}\n\nIMPORTANT: You are currently asking if the user wants to provide plan details. The user should say yes or no. Be friendly but guide them to say yes or no. Don't break character.",
                temperature=0.7,
                max_tokens=150
            )
    ...
    # If we're collecting details and the form has been submitted
    elif conversation_context["state"] == "collecting_details":
        # Check if the message indicates form submission
        if "submitted" in user_message or "details" in user_message:
            conversation_context["state"] = "recommending"
            return jsonify({
                "response": "Thank you for providing your details! Let me analyze this information and find the best plans for you.",
                "process_form": True  # Signal to the frontend to process the form data
            })
        else:
            # Use Llama to respond while waiting for form submission
            llama_response = llama_client.generate_response(
                prompt=f"User: {user_message}\nBlue:",
                system=f"{BLUE_SYSTEM_PROMPT}\n\nIMPORTANT: You are currently waiting for the user to submit their plan details through a form. Respond to their question but gently remind them to submit the form when they're ready.",
                temperature=0.7,
                max_tokens=150
            )
            return jsonify({"response": llama_response})

    # Handle when we're recommending plans
    elif conversation_context["state"] == "recommending":
        response = "I'm analyzing your requirements to find the best plans. Please wait a moment..."
        return jsonify({
            "response": response,
            "show_recommendations": True  # Signal to show recommendations
        })

    # Handle when we've shown recommendations and are in plan_selected state
    elif conversation_context["state"] == "plan_selected":
        # Use Llama to respond to questions about the selected plan
        llama_response = llama_client.generate_response(
            prompt=f"User: {user_message}\nBlue:",
            system=f"{BLUE_SYSTEM_PROMPT}\n\nIMPORTANT: The user has selected a plan. Respond to their questions about this plan or mobile plans in general.",
            temperature=0.7,
            max_tokens=200
        )
        return jsonify({"response": llama_response})

    # For any other state or casual conversation, leverage Llama's capabilities
    else:
        llama_response = llama_client.generate_response(
            prompt=f"User: {user_message}\nBlue:",
            system=BLUE_SYSTEM_PROMPT,
...
def submit_plan_details():
    ...
    # Update user_data with the form submission
    conversation_context["user_data"].update({
        "budget": float(data.get("current_price", 100.0)),
        "data_usage": float(data.get("current_data_usage", 5.0)),
    ...
    conversation_context["form_submitted"] = True
    conversation_context["state"] = "recommending"

    app.logger.info(f"Plan details submitted: {conversation_context['user_data']}")

    return jsonify({
        "response": "Details received! Let me find the best plans for you.",
        "show_recommendations": True  # Signal to the frontend to show recommendations
    })
...
def recommend_plan():
    ...
    # If we have no user data or plan info, return error
    if not user_data:
        return jsonify({"response": "No user data available. Please restart the conversation."})

    # Get user budget and data usage
    try:
        # Because we changed the CSV column names, we need to use the new column names
        # Get default values if missing
        budget = float(user_data.get('budget', 100))
        data_usage = float(user_data.get('data_usage', 5))

        # Check for matching plans using the updated column names
        matching_plans = plans_data[
            (plans_data["price"] <= budget) & (plans_data["data_gb"] >= data_usage)
            ]

        # If no plans match, try with relaxed criteria
        if matching_plans.empty:
            matching_plans = plans_data[plans_data["data_gb"] >= data_usage]

        # Still no matches, show some default options
        if matching_plans.empty:
            return jsonify({"response": "No suitable plan found. Adjust your criteria."})

        # Sort plans by price
        sorted_plans = matching_plans.sort_values(by=['price'])

        # Get top plans based on Config.MAX_RECOMMENDATIONS
        recommended = sorted_plans.head(Config.MAX_RECOMMENDATIONS).to_dict('records')

        # Store recommendations in context
        conversation_context["recommended_plans"] = recommended
        conversation_context["state"] = "displaying_recommendations"

        # Generate response using Llama to make it more conversational
        plan_info = "Based on your requirements, I've found these plans:\n\n"
        for i, plan in enumerate(recommended):
            plan_info += f"{i}. {plan['carrier']}: ${plan['price']}/month with {plan['data_gb']}GB data, {plan['text']} texting, and {plan['talk']} talk time\n"

        llama_prompt = f"""
        User needs a mobile plan with at least {data_usage}GB data and a budget of ${budget}/month.
        Here are the plans I found:

        {plan_info}

        Please create a friendly, helpful response summarizing these options. Keep it brief but informative.
        """
        ...
        # Create a nice descriptive response
        resp_text = f"Here are {len(recommended)} plans that match your needs:\n\n"
        for i, plan in enumerate(recommended):
            resp_text += f"{i}. {plan['carrier']}: ${plan['price']}/month with {plan['data_gb']}GB data\n"
        resp_text += "\nReply with the number of the plan you want to select, or ask me more about any specific plan."

        # Use Llama's response if it's good, otherwise use our fallback
        if len(llama_response) > 20 and "plan" in llama_response:
            resp_text = llama_response

        return jsonify({"response": resp_text})

    except Exception as e:
        app.logger.error(f"Error in recommend_plan: {str(e)}")
        app.logger.error(traceback.format_exc())
        return jsonify({"response": "Error fetching recommendations. Please try again."})
...
# -------------------------------------------------------------------------
#                      LLAMA INTEGRATION FOR BLUE
# -------------------------------------------------------------------------
class LlamaClient:
    """Client for interacting with Llama 3.2 via Ollama API"""

    def __init__(self, model_name="llama3.2-vision", api_url="http://localhost:11434"):
        self.model_name = model_name
        self.api_url = api_url

    def generate_response(self, prompt, system="", temperature=0.7, max_tokens=1024):
        ...
        try:
            ...
            app.logger.error(f"Llama API error: {str(e)}")
            return f"I'm having trouble thinking at the moment. Please try again later."


# Initialize the Llama client
llama_client = LlamaClient()

# System prompt that defines Blue's personality and knowledge base
BLUE_SYSTEM_PROMPT = """
You are Blue, an AI assistant specialized in mobile phone plans and telecommunications.

YOUR ROLE:
You help customers find the perfect mobile plan based on their needs, preferences, and budget. You are knowledgeable, friendly, and focused on delivering accurate information about mobile plans.

YOUR EXPERTISE INCLUDES:
- Canadian carriers, especially Koodo, Virgin, and Fido
- Mobile plan features (data packages, talk minutes, texting, international options)
- Phone plan costs, promotional offers, and hidden fees
- Comparing plans across different carriers
- Explaining terms like "BYOP" (Bring Your Own Phone)
- Answering questions about contract terms
- Providing money-saving tips
- 5G vs 4G/LTE network differences
- Special features like visual voicemail, call display, etc.

COMMUNICATION STYLE:
- Be conversational and friendly, but professional
- Provide concise, accurate information
- If you don't know something, admit it rather than making up information
- When comparing plans, highlight key differences in an easy-to-understand way
- Reference specific current plans when appropriate
- Use natural language, not sales jargon

IMPORTANT GUIDELINES:
1. Focus on providing objective advice based on customer needs
2. Don't push one carrier over others unless it clearly matches the customer's requirements
3. Answer basic questions about phones, but defer detailed technical support to the manufacturer
4. Stay updated with the latest plan information provided to you
5. If a user asks about completing a signup or application, guide them to the appropriate process
6. Your main goal is to help customers find the right plan and understand their options

Example questions you can answer:
- "Which carrier has the best unlimited data plans?"
- "What's the difference between Koodo and Fido plans?"
- "Do any plans include free international calls?"
- "What plans offer the most data for under $50?"
- "Can I keep my phone number if I switch carriers?"
"""


def get_detailed_plan_info(query_text):
    """
    Get more detailed plan information based on the user query.
    This helps Blue provide more accurate and detailed responses.
    """
    try:
        df = pd.read_csv('byop_plans.csv')

        # Determine what the user is asking about
        query_lower = query_text.lower()

        # Get relevant carrier information
        carriers = []
        if "koodo" in query_lower:
            carriers.append("Koodo")
        if "virgin" in query_lower:
            carriers.append("Virgin")
        if "fido" in query_lower:
            carriers.append("Fido")

        # If no specific carrier mentioned, include all
        if not carriers:
            carriers = ["Koodo", "Virgin", "Fido"]

        # Look for price range
        price_max = None
        if "under" in query_lower and "$" in query_lower:
            # Extract price after "$"
            dollar_index = query_lower.find("$")
            if dollar_index >= 0:
                price_text = query_lower[dollar_index + 1:].split()[0].strip()
                try:
                    price_max = float(price_text)
                except ValueError:
                    pass

        # Look for data requirements
        data_min = None
        if "at least" in query_lower and "gb" in query_lower:
            # Try to extract data amount
            gb_index = query_lower.find("gb")
            if gb_index >= 0:
                # Look backward from gb to find the number
                data_text = ""
                i = gb_index - 1
                while i >= 0 and (query_lower[i].isdigit() or query_lower[i] == '.'):
                    data_text = query_lower[i] + data_text
                    i -= 1
                try:
                    data_min = float(data_text)
                except ValueError:
                    pass

        # Apply filters to the dataframe
        filtered_df = df[df['carrier'].isin(carriers)]

        if price_max is not None:
            filtered_df = filtered_df[filtered_df['price'] <= price_max]

        if data_min is not None:
            filtered_df = filtered_df[filtered_df['data_gb'] >= data_min]

        # Sort by carrier and price
        filtered_df = filtered_df.sort_values(by=['carrier', 'price'])

        # Convert to dictionary records
        plans = filtered_df.to_dict('records')

        # Format the results
        results = "Detailed plan information based on your query:\n\n"

        current_carrier = None
        for plan in plans:
            if current_carrier != plan['carrier']:
                current_carrier = plan['carrier']
                results += f"\n{current_carrier.upper()} PLANS:\n"

            results += f"- ${plan['price']}/month: {plan['data_gb']}GB data, {plan['text']} texting, {plan['talk']} talk time"

            # Add any special features
            special_features = []
            if plan.get('unlimited_data', False):
                special_features.append("unlimited data")
            if plan.get('5g', False):
                special_features.append("5G access")
            if plan.get('international', False):
                special_features.append("international features")

            if special_features:
                results += f" (includes {', '.join(special_features)})"
        ...
        app.logger.error(f"Error getting detailed plan info: {str(e)}")
        return "I couldn't retrieve detailed plan information at the moment."


@app.route('/chat_with_blue', methods=['POST'])
def chat_with_blue():
    """Endpoint to directly chat with Blue powered by Llama"""
    data = request.json
    ...
    # Access the global conversation context to ensure state is maintained
    global conversation_context
    ...
    # Add the current message to past messages
    conversation_context["past_messages"].append({
        "role": "user",
        "content": user_message
    })

    # Initial greeting - only send this once when conversation is empty
    if conversation_context["state"] == "greeting":
        greeting = "Hello! Welcome to our mobile phone plan assistance service. I'm Blue, your AI assistant here to help you find the perfect mobile plan. Would you like to provide some details about what you're looking for?"
        conversation_context["state"] = "awaiting_yes_no"
        conversation_context["past_messages"].append({
            "role": "assistant",
            "content": greeting
    ...
    # Handle yes/no to the initial greeting prompt
    if conversation_context["state"] == "awaiting_yes_no":
        if user_message.lower() in ["yes", "y", "yeah", "sure", "ok"]:
            # User said yes - trigger the plan details dialog
            conversation_context["state"] = "collecting_details"
            response = "Great! I'll help you find the perfect plan. Could you please tell me about your current plan? For example, how much data do you use, and what's your monthly budget?"
            conversation_context["past_messages"].append({
                "role": "assistant",
                "content": response
            })
        ...
        elif user_message.lower() in ["no", "n", "nope"]:
            # User said no - ask if they have any questions about plans
            conversation_context["state"] = "casual_conversation"
            response = "I understand you don't want to provide details right now. Do you have any questions about mobile plans that I can help you with?"
            conversation_context["past_messages"].append({
                "role": "assistant",
                "content": response
            })
            ...
            # User didn't say yes or no clearly - guide them back
            response = "I'm not sure if you'd like to provide plan details. Could you please answer with a simple 'yes' or 'no'?"
            conversation_context["past_messages"].append({
                "role": "assistant",
                "content": response
            })
    ...
    # Handle plan details collection
    if conversation_context["state"] == "collecting_details":
        # Extract information from user's message using NLP
        try:
            # Use Llama to extract plan details from the message
            details_prompt = f"Extract the following information from the user's message: current price, data amount, and carrier. Message: {user_message}"
            details_response = llama_client.generate_response(
                prompt=details_prompt,
                system="Extract only the numerical values and carrier name. Format as JSON.",
                temperature=0.3,
                max_tokens=100
            )

            # Try to parse the extracted details
            try:
                details = json.loads(details_response)
                conversation_context["user_data"].update({
                    "current_price": float(details.get("price", 0)),
                    "current_data": float(details.get("data", 0)),
                    "current_carrier": details.get("carrier", "").lower()
                })
            except:
                # If parsing fails, use basic text analysis
                if "fido" in user_message.lower():
                    conversation_context["user_data"]["current_carrier"] = "fido"
                if "50" in user_message:
                    conversation_context["user_data"]["current_price"] = 50.0
                if "40" in user_message and "gb" in user_message.lower():
                    conversation_context["user_data"]["current_data"] = 40.0

            # Move to recommendations if we have enough information
            if all(key in conversation_context["user_data"] for key in
                   ["current_price", "current_data", "current_carrier"]):
                conversation_context["state"] = "recommending"
                response = "Thank you for providing those details! Let me analyze your current plan and find some options that might work better for you."
                conversation_context["past_messages"].append({
                    "role": "assistant",
                    "content": response
                })
                ...
                missing_info = []
                if "current_price" not in conversation_context["user_data"]:
                    missing_info.append("your current monthly payment")
                if "current_data" not in conversation_context["user_data"]:
                    missing_info.append("how much data you use")
                if "current_carrier" not in conversation_context["user_data"]:
                    missing_info.append("which carrier you're with")

                response = f"Could you please tell me about {', '.join(missing_info)}?"
                conversation_context["past_messages"].append({
                    "role": "assistant",
                    "content": response
                })
            ...
            app.logger.error(f"Error processing plan details: {str(e)}")
            response = "I'm having trouble understanding those details. Could you please provide them in a different way?"
            conversation_context["past_messages"].append({
                "role": "assistant",
                "content": response
            })
    ...
    # Handle plan selection and questions
    if conversation_context["state"] == "plan_selected":
        if "question" in user_message.lower() or "?" in user_message:
            # Use Llama to answer questions about the selected plan
            try:
                selected_plan = conversation_context.get("selected_plan", {})
                plan_context = f"The user has selected the following plan:\n"
                plan_context += f"Carrier: {selected_plan.get('carrier', 'N/A')}\n"
                plan_context += f"Price: ${selected_plan.get('price', 'N/A')}/month\n"
                plan_context += f"Data: {selected_plan.get('data_gb', 'N/A')}GB\n"
                plan_context += f"Talk: {selected_plan.get('talk', 'N/A')} minutes\n"
                plan_context += f"Text: {selected_plan.get('text', 'N/A')} messages\n"

                llama_response = llama_client.generate_response(
                    prompt=f"User question: {user_message}\nPlan details: {plan_context}\nBlue:",
                    system=f"{BLUE_SYSTEM_PROMPT}\n\nIMPORTANT: The user has selected a specific plan and is asking questions about it. Provide accurate information based on the plan details.",
                    temperature=0.7,
                    max_tokens=200
                )
                ...
                app.logger.error(f"Error answering plan question: {str(e)}")
                response = "I apologize, but I don't have enough information about the plan at the moment. Would you like to proceed with finalizing your plan selection?"
        else:
            # If no question, ask about finalizing the plan
            response = "Would you like to proceed with finalizing your plan selection?"
    ...
    if conversation_context["state"] == "finalizing":
        if user_message.lower() in ["yes", "y", "yeah", "sure", "ok"]:
            conversation_context["state"] = "checkout"
            response = "Great! I'll direct you to the checkout page to complete your plan selection."
            conversation_context["past_messages"].append({
                "role": "assistant",
                "content": response
            })
            ...
            response = "Would you like to explore other plans or do you have any other questions?"
            conversation_context["past_messages"].append({
                "role": "assistant",
                "content": response
            })
    ...
    # For all other messages, use Llama's intelligence but respect the conversation state
    try:
        # Create a context-rich system prompt with plan details
        plan_context = "Here is the current mobile plan information:\n\n"

        # Add the plans information
        df = pd.read_csv('byop_plans.csv')
        koodo_plans = df[df['carrier'] == 'Koodo'].to_dict('records')
        virgin_plans = df[df['carrier'] == 'Virgin'].to_dict('records')
        fido_plans = df[df['carrier'] == 'Fido'].to_dict('records')

        for carrier, plans in [('KOODO', koodo_plans), ('VIRGIN', virgin_plans), ('FIDO', fido_plans)]:
            plan_context += f"\n{carrier} PLANS:\n"
            for i, plan in enumerate(plans[:5]):
                plan_context += f"- ${plan['price']}/month: {plan['data_gb']}GB data, {plan['text']} texting, {plan['talk']} talk time\n"

        # Create an enhanced system prompt that respects the current state
        state_info = f"Current conversation state: {conversation_context['state']}\n"
        context_info = ""

        if conversation_context.get("recommended_plans", []):
            context_info += "Currently recommended plans:\n"
            for i, plan in enumerate(conversation_context["recommended_plans"]):
                context_info += f"{i}. {plan['carrier']}: ${plan['price']}/month with {plan['data_gb']}GB data\n"

        enhanced_system_prompt = f"{BLUE_SYSTEM_PROMPT}\n\n{state_info}{context_info}\n\n{plan_context}"

        # Build a conversation history for context
        conversation = ""
        for msg in conversation_context["past_messages"][-5:]:  # Keep only the last 5 messages for context
            if msg["role"] == "user":
                conversation += f"User: {msg['content']}\n"
            else:
                conversation += f"Blue: {msg['content']}\n"

        # Add the current message and prepare the prompt
        conversation += f"User: {user_message}\nBlue:"

        # Get the response from Llama
        response = llama_client.generate_response(
            prompt=conversation,
        ...
        app.logger.error(f"Error in chat_with_blue: {str(e)}")
        app.logger.error(traceback.format_exc())
...
# -------------------------------------------------------------------------
#                      LLAMA INTEGRATION FOR BLUE - UI
# -------------------------------------------------------------------------

@app.route('/blue_chat_interface', methods=['GET'])
def blue_chat_interface():
    """Serves a simple HTML interface for chatting with Blue"""
    html = """
    <!DOCTYPE html>
    <html>
    <head>
        <title>Chat with Blue - Mobile Plan Assistant</title>
        ...
        <style>
            ...
            .assistant-container .message-content {
                background-color: #f0f0f0;
                border-bottom-left-radius: 4px;
                margin-right: 10px;
            }
    ...
    <body>
        <div class="container">
            <div class="header">
                ...
                <p>Your AI Mobile Plan Assistant</p>
            </div>
            <div class="chat-container">
                <div class="chat-messages" id="chat-messages">
                    <div class="message-container assistant-container">
                        <div class="blue-avatar">B</div>
                        <div class="message-content">
                            Hi, I'm Blue! I can help you find the perfect mobile plan. What are you looking for today?
                        </div>
                    </div>
                </div>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                function addMessage(message, isUser = false) {
                    ...
                    messageContainer.className = isUser ? 'message-container user-container' : 'message-container assistant-container';

                    const avatar = document.createElement('div');
                    avatar.className = isUser ? 'user-avatar' : 'blue-avatar';
                ...
                // Function to send message to Blue
                async function sendMessageToBlue(message) {
                    // Show typing indicator
                    typingIndicator.style.display = 'block';

                    try {
                        const response = await fetch('/chat_with_blue', {
                            method: 'POST',
                            headers: {
                        ...
                        if (data.status === 'success') {
                            ...
                            // Add Blue's response to the chat
...
def index():
    """Home page with links to various parts of the application"""
    html = """
    <!DOCTYPE html>
    <html>
    <head>
        <title>Mobile Plan Assistant</title>
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <style>
            body {
                font-family: 'Arial', sans-serif;
    ...
    <body>
        <div class="container">
            <div class="header">
                <h1>Mobile Plan Assistant</h1>
                <p>Find and compare the best mobile plans for your needs</p>
            </div>
            <div class="content">
                <div class="feature-card">
                    <h2>Chat with Blue <span class="new-badge">NEW</span></h2>
                    <p>Talk to our AI assistant Blue to get personalized recommendations and answers about mobile plans.</p>
                    <p>Ask anything about plans, carriers, features, and get intelligent, helpful responses.</p>
                    <a href="/blue_chat_interface" class="btn">Start Chatting</a>
                </div>

                <div class="feature-card">
                    <h2>Plan Search</h2>
                    <p>Search for mobile plans based on your specific requirements.</p>
                    <p>Filter by data usage, price, carrier, and more to find the perfect match.</p>
                    <a href="/search" class="btn">Search Plans</a>
                </div>

                <div class="feature-card">
                    <h2>Plan Recommendations</h2>
                    <p>Get personalized plan recommendations based on your usage patterns.</p>
                    <p>Tell us what you need and we'll find the best options for you.</p>
                    <a href="/recommend_plan" class="btn">Get Recommendations</a>
                </div>
            </div>
        </div>
    </body>
