...
function generateAccessibilityTree({
  ...
  function addTextChild(text, parentNode, childNode, childrenArray, iFramePath) {
    ...
    const node = createNode(span, iFramePath);
    if (node) {
      ...
      if (childNode.nodeType === Node.TEXT_NODE) {
        ...
        if (text) {
          if (childNodes.length > 1) {
            addTextChild(text, node, childNode, children, newIframePath);
          } else {
            ...
            if (
              !name ||
              (node.tagName && node.tagName.toLowerCase() === 'button')
            ) {
              // If the parent has no name, set name to the child text.
              // Special case: textContent for button elements
              name = text;
            } else {
              // If the parent does have a name attribute, retain the child text as a separate node.
              addTextChild(text, node, childNode, children, newIframePath);
            }
          }
        }
