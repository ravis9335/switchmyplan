...
async def bell_flow_full(session_id: str, user_data: dict, plan_info: dict):
    ...
    # Get activation type (physical SIM or eSIM)
    activation_type = user_data.get("activation_type", "physical")
    imei_number     = user_data.get("imei", "")
    sim_number      = user_data.get("sim", "")
    ...
    try:
       ...
       # STEP X) Branch based on activation_type
        if activation_type.lower() == "esim":
            print("Customer chose eSIM activation")
            try:
                # wait for the SIM selection section to appear
                await page.wait_for_selector("text=SIM selection", timeout=10000)

                # locate the eSIM radio by role/name and check it
                esim_radio = page.get_by_role("radio", name=re.compile(r"eSIM", re.IGNORECASE))
                await esim_radio.check(force=True)
                print("✔️ Checked eSIM radio button")

                # now wait for the IMEI field to show up and fill it
                await page.wait_for_selector("#imei-number", timeout=5000)
                await page.fill("#imei-number", imei_number)
                await page.wait_for_timeout(5000)
                print(f"✔️ Filled IMEI number: {imei_number}")

            except Exception as e:
                print(f"❌ Error selecting eSIM or filling IMEI: {e}")

        else:
            print("Customer chose physical SIM activation")
            try:
                # wait for SIM selection to appear
                await page.wait_for_selector("text=SIM selection", timeout=10000)

                # locate the SIM‑card radio by role/name and check it
                sim_radio = page.get_by_role("radio", name=re.compile(r"SIM\s*card", re.IGNORECASE))
                await sim_radio.check(force=True)
                print("✔️ Checked SIM card radio button")

                # Fill SIM card number if provided
                if sim_number and len(sim_number.strip()) > 0:
                    await page.wait_for_selector("#sim-number", timeout=5000)
                    await page.fill("#sim-number", sim_number)
                    print(f"✔️ Filled SIM number: {sim_number}")

            except Exception as e:
                print(f"❌ Error selecting SIM card: {e}")
...
# Arrow down to highlight the first suggestion, then enter to select
        ...
        # Possibly "Confirm"
        try:
            # option A: by data‑dtname attribute (most precise)
            await page.click(
                '[data-dtname="Confirm information button on Checkout Personal Info step"]',
            force=True
        )
        except Exception:
            # option B: by role + name
            confirm_btn = page.get_by_role("button", name="Confirm")
            await confirm_btn.click(force=True)

        print("✔️ Clicked 'Confirm' on popup")
        await page.wait_for_timeout(10000)

        # Possibly fill phone number preference (transfer vs new), etc.
        if number_preference.lower() == "transfer":
            # 1) Select the "Transfer your current number to Bell" radio
            await page.locator("input#current-option-2").check(force=True)
            print("✔️ Selected 'Transfer your current number to Bell'")
            await page.wait_for_timeout(500)

            # 2) Fill in the transfer number
            await page.get_by_label("Phone number to transfer").fill(transfer_number)
            print(f"✔️ Entered transfer number: {transfer_number}")
            await page.wait_for_timeout(500)

            # 3) Click "Verify transferability"
            await page.get_by_role(
                "button",
                name=re.compile(r"Verify transferability", re.IGNORECASE)
            ).click(force=True)
            print("✔️ Clicked 'Verify transferability'")
            await page.wait_for_timeout(5000)

            # 4) Check the T&C box
            # 4) Check the Bell T&C box by its real id
            try:
                # 4a) Wait for the checkbox to land in the DOM
                await page.wait_for_selector("#i-authorize", timeout=10000)

                # 4b) Scroll it into view (if it's off‑screen)
                terms_checkbox = page.locator("#i-authorize")
                await terms_checkbox.scroll_into_view_if_needed()

                # 4c) Use .check() so Playwright fires the proper checkbox events
            ...
            await page.get_by_role(
                "button",
                name=re.compile(r"Confirm number transfer", re.IGNORECASE)
            ).click(force=True)
            print("✅ Clicked 'Confirm number transfer'")
            await page.wait_for_timeout(5000)

        else:
            # 1) Select the "Select a new number" radio
            await page.locator("input#current-option-1").check(force=True)
            print("✔️ Selected 'Select a new number'")
            await page.wait_for_timeout(500)

            # 2) Click the "Continue" button once the new‑number form is ready
            await page.get_by_role(
                "button",
                name=re.compile(r"Continue(?! to Number setup)", re.IGNORECASE)
            ).click(force=True)
            print("✔️ Clicked 'Continue' for new number")
            await page.wait_for_timeout(5000)
...
def select_plan():
    ...
    # Check if the selected plan is from Virgin
    if carrier.lower() == 'virgin':
        # Redirect to Virgin's BYOP plans page
        return jsonify({
            "success": True,
            "redirect": True,
...
def checkout():
    ...
    # Try to override from your CSV if available
    ...
    if isinstance(plans_df, pd.DataFrame):
        ...
        if not mp.empty:
            plan_features = mp.iloc[0]['plan_features'] or ""

    # Calculate tax & totals
    tax_amount    = round(plan_price * 0.13, 2)
    total_monthly = plan_price + tax_amount

    # Build exactly the same features_list your card‑builder JS uses for Bell/Fido/Rogers/Telus
    carrier_title = carrier.title()
    features_list = []

    if carrier_title in ['Bell']:
        # split on newlines, strip bullets and leading "and "
        for line in plan_features.split('\n'):
            clean = line.strip()
            clean = re.sub(r'^[•\-\s]+', '', clean)
            clean = re.sub(r'^and\s+', '', clean, flags=re.IGNORECASE).strip()
            if clean:
                features_list.append({ 'text': clean, 'included': True })
        # fallback when CSV had no features
        if not features_list:
            features_list = [
                {'text': 'Canada‑wide Calling',  'included': True},
                {'text': 'Unlimited Texting',     'included': True},
                {'text': 'Data Access',           'included': bool(plan_data and plan_data!='0')}
            ]
    else:
        # (You only do Bell today, so you won't hit this branch
        #  but here's how you'd extend for Virgin, Freedom, Koodo…)
        if carrier_title == 'Virgin':
            features_list = [
                ...
                {'text':'Data Access',                                       'included': bool(plan_data and plan_data!='0')},
                {'text':'Visual Voicemail','included':True},
                {'text':'Call Display','included':True},
                {'text':'Call Waiting','included':True},
                {'text':'Conference Calling','included':True},
                {'text':'Call Forwarding','included':True},
            ]
        # … add more carriers if you ever need to …
...
def checkout_submit():
    ...
    data = {
        ...
        "activation_type": form_data.get("activation_type", "physical")
